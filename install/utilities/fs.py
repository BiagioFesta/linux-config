...
def create_dir(directory: str,
               ...
               prefix_home: bool = True):
    directory = os.path.join(home_dir(), directory) \
        if prefix_home else directory
    ...
    if is_path_exists(directory, prefix_home=False):
        ...
        ans = input("  The directory '{}' already exists. "
                    "You wanna CONTINUE? [yN]: ".format(directory))
        if ans != 'y' and ans != 'Y':
...
def create_symlink(src: str,
                   dest: str,
                   prefix_repo: bool = True,
                   prefix_home: bool = True):
    ...
    if not is_file_exists(src, prefix_home=False):
        raise Exception("The file to install ('{}') does not exist"
                        .format(src))

    if is_file_exists(dest, prefix_home=False):
        ans = input("  Destination ('{}') already exists. "
                    "You wanna OVERRIDE? [yN]: ".format(dest))
        if ans != 'y' and ans != 'Y':
            return
...
def ensure_file(filename: str, prefix_home: bool = True):
    ...
    if not is_file_exists(filename, prefix_home=False):
        print("The file '{}' does not exist. I am gonna create it"
              .format(filename))
